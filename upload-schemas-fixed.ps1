...
try {
    ...
    Write-Host "[OK] Invoice Schema uploaded: $($response.schemaId)"
    Write-Host "   Title: $($response.title)"
    Write-Host "   Fields: $($response.fieldCount)"
} catch {
    Write-Host "[ERR] Error uploading invoice schema: $_"
}

# Upload API Response Schema  
Write-Host ""
Write-Host "[*] Uploading API Response Schema..."
...
try {
    ...
    Write-Host "[OK] API Response Schema uploaded: $($response2.schemaId)"
    Write-Host "   Title: $($response2.title)"
    Write-Host "   Fields: $($response2.fieldCount)"
} catch {
    Write-Host "[ERR] Error uploading API response schema: $_"
}

# List all schemas
Write-Host ""
Write-Host "Listing all schemas:"
try {
    $schemas = Invoke-RestMethod -Uri "$apiUrl/api/schema/list" -UseBasicParsing
    Write-Host "Total: $($schemas.data.Count) schemas"
    foreach ($s in $schemas.data) {
        Write-Host "  - $($s.title) (ID: $($s.id), Fields: $($s.fieldCount))"
    }
} catch {
    Write-Host "[ERR] Error listing schemas: $_"
}
