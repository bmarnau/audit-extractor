...
## � Example-Schemas & Testdaten

Dieses Projekt enthält zwei vorkonfigurierte Beispiel-Schemas zur schnellen Inbetriebnahme und zum Testen der Extraction-Funktionalität. Diese Dateien sind **Teil des Projekts** und werden nicht gelöscht.

### Verzeichnis-Struktur

```
extractor/
├── example-schemas/              # ✅ Versionskontrolliert (in Git)
│   ├── .gitkeep                 # Sichert Verzeichnis vor Löschung
│   ├── README.md                # Ausführliche Dokumentation
│   ├── invoice-schema.json      # Schema 1: Rechnungen
│   ├── invoice-example-1.json   # Beispiel-Daten zu Schema 1
│   ├── api-response-schema.json # Schema 2: REST API Responses
│   └── api-response-example-1.json # Beispiel-Daten zu Schema 2
└── ...
```

**Wichtig:** Die `.gitkeep`-Datei stellt sicher, dass das `example-schemas/`-Verzeichnis auch mit Docker-Rebuilds oder Maintenance nicht versehentlich gelöscht wird.

### Schema 1: Invoice Extraction (Dokument-basiert)

**Dateien:**
- `example-schemas/invoice-schema.json` — JSON Schema für Rechnungsstruktur
- `example-schemas/invoice-example-1.json` — Beispiel-Rechnungsdaten

**Felder:**
- Rechnungsnummer, Datum, Fälligkeitsdatum
- Rechnungssteller & -empfänger (Name, Adresse, Steuernummer)
- Rechnungspositionen (Beschreibung, Menge, Preis)
- Summen: Zwischensumme, Steuern, Gesamtbetrag
- Zahlungsdetails & Bankkonten

**Anwendungsfall:** Automatische Extraktion von Rechnungsdaten aus PDF/Word-Dokumenten

**Confidence-Erwartung:** 0.90-0.98 (hohe Genauigkeit durch strukturiertes Format)

### Schema 2: REST API Response Extraction (JSON-basiert)

**Dateien:**
- `example-schemas/api-response-schema.json` — JSON Schema für API-Response-Struktur
- `example-schemas/api-response-example-1.json` — Beispiel-API-Response

**Felder:**
- Request-Metadaten: Endpoint, Methode, Zeitstempel, Response-Code
- Benutzer-Info: ID, Name, E-Mail, Rolle, Abteilung
- Response-Daten: Datensätze, Pagination, Fehler
- Compliance-Flags: GDPR, Verschlüsselung, Audit-Logging
- Request-IDs für Tracing

**Anwendungsfall:** Compliance & Audit-Logging von REST API Calls

**Confidence-Erwartung:** 0.93-0.99 (sehr hohe Genauigkeit für strukturierte JSON)

### Schnellstart mit Example-Schemas

#### 1. Schema hochladen

```bash
# Option A: Über curl (Backend API)
curl -X POST http://localhost:3000/api/schema/upload \
  -F "schema=@example-schemas/invoice-schema.json" \
  -F "examples=@example-schemas/invoice-example-1.json" \
  -F "schemaName=invoice-production"
...
#### 2. Extraktionsregeln generieren

```bash
curl -X POST http://localhost:3000/api/schema/uuid-12345/generate-rules \
  -H "Content-Type: application/json" \
  -d '{
    "aggressiveness": 0.8,
    "customKeywords": ["invoice", "total", "amount", "vendor", "customer"]
  }'
...
Die generierten Regeln können nun auf echte Dokumente angewandt werden:

```bash
curl -X POST http://localhost:3000/api/extract/pdf \
  -F "file=@my-invoice.pdf" \
  -F "schemaId=uuid-12345" \
  -F "includeConfidence=true"
```

### Frontend-Integration

Statt curl können Sie auch die **React-UI** verwenden:

1. Öffnen Sie **http://localhost** im Browser
2. Navigieren Sie zu **Schema Management** → **Upload Schema**
3. Wählen Sie `example-schemas/invoice-schema.json`
4. Laden Sie `example-schemas/invoice-example-1.json` als Beispiel hoch
5. Klicken Sie **"Generate Rules"**
6. Überprüfen Sie die generierten Extraktionsregeln
7. Speichern Sie die Regeln zur Wiederverwendung
...
```bash
# 1. JSON Schema-Datei erstellen
cat > example-schemas/custom-schema.json << 'EOF'
{
...
# 2. Beispiel-Datei erstellen
cat > example-schemas/custom-example-1.json << 'EOF'
{
  "fieldName": "Example Value",
  "fieldAmount": 12345.67
}
EOF

# 3. Hochladen und testen (wie oben)
```

**Verzeichnis sauber halten:**

```bash
# Alte Beispiele archivieren (nicht löschen!)
mkdir -p example-schemas/archive
mv example-schemas/old-schema.json example-schemas/archive/

# Liste aller verfügbaren Schemas
ls -la example-schemas/*.json
```

### Git-Integration

Die Example-Schemas sind Teil des Git-Repositories:

```bash
# Status überprüfen
git status example-schemas/

# Änderungen commiten
git add example-schemas/
git commit -m "Add example schemas for testing"

# Änderungen pushen
git push origin main

# Klone mit Beispielen (automatisch enthalten)
git clone <repository-url>
cd extractor
# example-schemas/ ist bereits enthalten!
```

### Richtlinien

✅ **ERLAUBT:**
- Beispiele bearbeiten und verbessern
- Neue Beispiele hinzufügen
- Korrekturen an Schemas vornehmen
- In Git committen

⚠️ **VORSICHT:**
- Nicht komplett löschen (`.gitkeep` bleibt!)
- Nicht in `.gitignore` eintragen (Verzeichnis würde nicht geklont)
- Nicht mit sensiblen Daten füllen
- Dateigrößen moderat halten (<5MB pro Datei)

❌ **NICHT ERLAUBT:**
- Beispiele in `.gitignore` eintragen
- Verzeichnis manuell löschen
- Große Binärdateien speichern
- Sensitive Produktionsdaten verwenden
