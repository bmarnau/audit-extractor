 ...
 *
 * Unterstützt Custom Schema Format (wie in extraction-rules/schemas/)
 */
...
import { ValidationServiceError } from '@domain/validation/ValidationService';

describe('AjvValidationService', () => {
  let service: AjvValidationService;
  ...
  beforeAll(async () => {
    ...
    // Create test schemas in Custom Format (wie extraction-rules/schemas/)
    const invoiceSchema = {
    ...
    // Write schemas with versioned filenames (wie im Real-System)
    await fs.writeFile(
      path.join(testSchemaDir, 'invoice-v1.0.0.json'),
      JSON.stringify(invoiceSchema)
    );

    await fs.writeFile(
      path.join(testSchemaDir, 'contract-v1.0.0.json'),
      JSON.stringify(contractSchema)
    );
  });

  beforeEach(() => {
    service = new AjvValidationService(testSchemaDir);
  });
  ...
  describe('loadSchema', () => {
    ...
    it('should handle versioned schema filenames', async () => {
      // Datei heißt "invoice-v1.0.0.json", aber loadSchema('invoice') sollte es finden
      const schema = await service.loadSchema('invoice');
  ...
  describe('validate', () => {
    ...
    it('should detect missing required fields', async () => {
      const result: ExtractionResult = {
        extractedFields: new Map([
          ['invoiceNumber', { value: 'INV-001', confidence: 0.95, sources: [] }],
          ['invoiceDate', { value: '2024-01-15', confidence: 0.95, sources: [] }],
          // Missing: totalAmount, customerName
        ]),
        missingFields: [],
        warnings: [],
      };
    ...
    it('should allow optional fields to be missing', async () => {
      const result: ExtractionResult = {
        extractedFields: new Map([
          ...
          ['totalAmount', { value: 1000, confidence: 0.95, sources: [] }],
          ['customerName', { value: 'Acme Corp', confidence: 0.95, sources: [] }],
          // dueDate is optional, OK to miss
        ]),
        missingFields: [],
        warnings: [],
      };
    ...
    it('should detect type mismatches', async () => {
      const result: ExtractionResult = {
        extractedFields: new Map([
          ['invoiceNumber', { value: 'INV-001', confidence: 0.95, sources: [] }],
          ['invoiceDate', { value: '2024-01-15', confidence: 0.95, sources: [] }],
          ['totalAmount', { value: 'not-a-number', confidence: 0.95, sources: [] }], // Wrong type!
          ['customerName', { value: 'Acme Corp', confidence: 0.95, sources: [] }],
        ]),
        missingFields: [],
        warnings: [],
    ...
    it('should throw error for unknown schema', async () => {
      ...
      await expect(service.validate(result, 'unknown-type')).rejects.toThrow(
  ...
  describe('getSupportedDocumentTypes', () => {
    ...
    it('should deduplicate document types', async () => {
      // Falls mehrere Versionen existieren, sollte jeder Typ nur einmal vorkommen
      const types = await service.getSupportedDocumentTypes();
  ...
  describe('No Data Generation', () => {
    it('should not add default values to result', async () => {
      ...
      // Validation sollte result nicht modifizieren
      expect(result.extractedFields.size).toBe(4);
      expect(validation.missingFields).not.toContain('dueDate'); // Nicht hinzugefügt!
    });
    ...
    it('should not invent missing fields', async () => {
      ...
      // missingFields sollte nur echte Fehlende enthalten, nicht erfundene
      expect(validation.missingFields).toEqual(
        expect.arrayContaining(['invoiceDate', 'totalAmount', 'customerName'])
      );
