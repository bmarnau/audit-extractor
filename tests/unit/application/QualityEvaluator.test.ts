...
import { QualityEvaluatorError } from '@application/quality/QualityEvaluator';
...
describe('MetricsBasedQualityEvaluator', () => {
  let evaluator: MetricsBasedQualityEvaluator;

  beforeEach(() => {
    evaluator = new MetricsBasedQualityEvaluator();
  });
  ...
  describe('evaluate', () => {
    ...
    it('should evaluate result without sources', async () => {
      ...
      expect(score.verifiability).toBe(0.5); // 1 of 2 has sources
  ...
  describe('evaluateSchemaConformity', () => {
    ...
    it('should validate against schema', () => {
      const evaluatorWithSchema = new MetricsBasedQualityEvaluator({
        schema: {
          properties: {
      ...
      expect(score).toBe(0.5); // 1 of 2 fields in schema
    });
  });
  ...
  describe('Weight Configuration', () => {
    it('should reject weights that do not sum to 1.0', () => {
      expect(() => {
        new MetricsBasedQualityEvaluator({
          weights: {
            completeness: 0.5,
            consistency: 0.3,
            verifiability: 0.1,
            schemaConformity: 0.05, // Total = 0.95
          },
        });
      }).toThrow(QualityEvaluatorError);
    });

    it('should accept valid weights', () => {
      const eval2 = new MetricsBasedQualityEvaluator({
        weights: {
          completeness: 0.4,
          consistency: 0.3,
          verifiability: 0.2,
